...
def render_batch_interface(llm_server):
    ...
    # Prefetch in background using st.experimental_singleton + callback
    ...
    if not st.session_state.prefetched_batch and not st.session_state.is_prefetching:
        st.markdown("🔄 Downloading next batch... if you dont see any Submition button please wait a few seconds ⏳")
        trigger_prefetch()
